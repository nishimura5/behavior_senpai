...
class App(ttk.Frame):
    ...
    def draw(self):
        ...
        plot_df = pd.concat([tar_df, speed_df], axis=1)
        self.current_dt_span = dt_span

        # make export data
        calc_df = plot_df.loc[pd.IndexSlice[:, :, current_keypoint], :].droplevel(2)
        calc_df = calc_df.drop(columns="timestamp").add_suffix(f"({current_keypoint})")
        self.calc_df = pd.concat([self.calc_df, calc_df], axis=1)
